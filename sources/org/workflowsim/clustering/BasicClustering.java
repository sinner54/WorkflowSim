...
public class BasicClustering implements ClusteringInterface {
    ...
    protected final Job addTasks2Job(List<Task> taskList) {
        if (taskList != null && !taskList.isEmpty()) {
            ...
            //聚任务时，选择最大的taskid作为新的id
            Integer maxTaskId = 0;
            for (Task task : taskList) {
                if (maxTaskId < task.getCloudletId()){
                    maxTaskId = task.getCloudletId();
                }
            }
            Job job = new Job(maxTaskId,length);
            ...
            for (Task task : taskList) {
                ...
                submitTime = task.getArrivalTime();
            }
